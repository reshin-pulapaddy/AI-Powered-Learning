...
export default function MegaMenu({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div
      ref={menuRef}
      className="fixed left-1/2 -translate-x-1/2 z-40 bg-white shadow-2xl border border-gray-200 rounded-xl hidden md:block animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden"
      style={{ 
        top: '90px',
        width: '70%',
        height: '70vh',
    ...
    >
      <div className="h-full overflow-hidden flex flex-col">
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-0 h-full">
          ...
          <div className="bg-gradient-to-b from-gray-50 to-gray-100 border-r border-gray-200 overflow-y-auto custom-scrollbar">
            <div className="p-4">
                  ...
                  >
                    <div className="flex items-start gap-2.5">
                      <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-all ${
                        selectedProduct?.id === product.id
                          ? 'bg-white/20'
                          : 'bg-gray-100 group-hover:bg-[#14467b]/10'
                      }`}>
                        {product.icon}
                      </div>
          ...
          <div className="p-8 overflow-y-auto custom-scrollbar h-full bg-white">
            {selectedProduct ? (
              <div
                ...
                {selectedProduct.features.sections.length > 0 ? (
                  <div className="grid grid-cols-2 gap-6">
                    {selectedProduct.features.sections.map((section, index) => (
                      <div 
                        key={index} 
                        className={index % 2 === 0 ? 'pr-6 border-r border-gray-200' : 'pl-6'}
                      >
                        <h3 className="text-base font-bold text-gray-900 mb-3">
                          <Link 
                            href={`#${selectedProduct.id}-${section.title.toLowerCase().replace(/\s+/g, '-').replace(/&/g, '')}`}
                            className="hover:text-[#14467b] transition-colors"
                          >
                            {section.title}
                          </Link>
                        </h3>
                        <ul className="space-y-2">
                          {section.items.map((item, itemIndex) => (
                            <li key={itemIndex} className="flex items-start gap-2">
                              <span className="mt-1.5 flex h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#14467b]"></span>
                              <Link
                                href={`#${selectedProduct.id}-${item.toLowerCase().replace(/\s+/g, '-').replace(/&/g, '').replace(/\(/g, '').replace(/\)/g, '')}`}
                                className="text-sm text-gray-700 hover:text-[#14467b] transition-colors"
                              >
                                {item}
                              </Link>
                            </li>
